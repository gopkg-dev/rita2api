...
- `RATI_DB_PATH`：SQLite 文件路径，默认 `data/rita.db`
...
- `RITA_COOKIE_NAME`：匿名会话 Cookie 名称，默认 `rita_session`

## Docker

```bash
docker build -t rita-ai-studio .
docker run --rm -p 8080:8080 -v $(pwd)/data:/app/data rita-ai-studio
```
